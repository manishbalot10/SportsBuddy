            ...
            {cluster.player && (
              ...
              >
                <div style={{ padding: '8px 12px', minWidth: '160px' }}>
                  ...
                  {/* Primary Sports with Rating Dots */}
                  {(cluster.player.primarySports || [{ sport: cluster.player.sport, level: 4 }]).slice(0, 1).map((ps, idx) => (
                    <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '4px' }}>
                      <span style={{ fontSize: '12px', color: '#333', fontWeight: 500 }}>{ps.sport}</span>
                      <div style={{ display: 'flex', gap: '2px' }}>
                        {[1,2,3,4,5].map(dot => (
                          <div key={dot} style={{ width: '8px', height: '8px', borderRadius: '50%', backgroundColor: dot <= ps.level ? '#E17827' : '#DBDBDB' }} />
                        ))}
                  ...
                  {/* Secondary Sports */}
                  {cluster.player.secondarySports && cluster.player.secondarySports.length > 0 && (
                    <div style={{ fontSize: '11px', color: '#666', marginTop: '4px' }}>
                      {cluster.player.secondarySports.slice(0, 2).join(' • ')}
                    </div>
                  )}
...
export const ScalableMap: React.FC<ScalableMapProps> = ({ 
  ...
  return (
    <div className="w-full h-full z-0 relative">
      ...
      >
        <svg width="160" height="42" viewBox="0 0 233 62" fill="none" xmlns="http://www.w3.org/2000/svg">
