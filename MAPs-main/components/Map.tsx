...
interface ClusterItem {
  ...
  player?: Player;
  opacity?: number; // For visual indicator mode
...
const ViewportDataFetcher: React.FC<{
  ...
  maxDistance?: number;
}> = ({ onDataLoad, sport, userType, userLocation, maxDistance }) => {
  ...
  const fetchViewportData = useCallback(() => {
    ...
    if (userType && userType !== 'all') {
      params.append('role', userType === 'players' ? 'player' : 'coach');
    }

    // Add distance filtering parameters if user location is available
  ...
  }, [map, sport, userType, userLocation, maxDistance, onDataLoad]);
            ...
            >
              <Popup>
                <div style={{ minWidth: '150px', padding: '4px' }}>
                  ...
                  {(player.primarySports || [{ sport: player.sport, level: 4 }]).slice(0, 1).map((ps, idx) => (
                    <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '4px' }}>
                      <span style={{ fontSize: '12px', color: '#333' }}>{ps.sport}</span>
                      <div style={{ display: 'flex', gap: '2px' }}>
                        {[1, 2, 3, 4, 5].map(dot => (
                          <div key={dot} style={{ width: '8px', height: '8px', borderRadius: '50%', backgroundColor: dot <= ps.level ? '#E17827' : '#DBDBDB' }} />
                        ))}
                      </div>
                    </div>
                  ))}
                  {/* Secondary Sports */}
                  {player.secondarySports && player.secondarySports.length > 0 && (
                    <div style={{ fontSize: '11px', color: '#666' }}>{player.secondarySports.slice(0, 2).join(' • ')}</div>
                  )}
...
interface ScalableMapProps {
  ...
  sport?: string;
  viewMode: 'players' | 'coaches';
  maxDistance?: number; // Search radius in kilometers
  userType?: 'all' | 'players' | 'coaches';
}
...
// Zoom Control - Zoom In/Out buttons for web view
const ZoomControl: React.FC = () => {
  ...
  useEffect(() => {
    // Only render on desktop/web view
    if (!isDesktop) return;
    ...
    // Add hover effects
    const zoomInBtn = container.querySelector('.zoom-in-btn') as HTMLElement;
    const zoomOutBtn = container.querySelector('.zoom-out-btn') as HTMLElement;

    if (zoomInBtn) {
      zoomInBtn.onmouseenter = () => { zoomInBtn.style.backgroundColor = '#f3f4f6'; };
      zoomInBtn.onmouseleave = () => { zoomInBtn.style.backgroundColor = 'white'; };
      zoomInBtn.onclick = (e) => {
        e.preventDefault();
        e.stopPropagation();
        map.zoomIn();
    ...
    if (zoomOutBtn) {
      zoomOutBtn.onmouseenter = () => { zoomOutBtn.style.backgroundColor = '#f3f4f6'; };
      zoomOutBtn.onmouseleave = () => { zoomOutBtn.style.backgroundColor = 'white'; };
    ...
    const control = new ZoomControlClass({ position: 'bottomright' });
    control.addTo(map);
...
const ScalableMarkers: React.FC<{
...
}> = ({ clusters, onPlayerSelect, userLocation, maxDistance, onCountUpdate }) => {
  const map = useMap();

  const handleClusterClick = (cluster: ClusterItem) => {
    // Dismiss any open player card when navigating via cluster
    onPlayerSelect(null);
  ...
  // Calculate players within distance and apply opacity
  const { processedClusters, withinDistanceCount } = React.useMemo(() => {
    let count = 0;

    const processed = clusters.map(cluster => {
      if (cluster.isCluster) {
        return { ...cluster, opacity: 1.0 };
      }

      // For individual players, calculate distance and opacity
      if (cluster.player && userLocation && maxDistance && maxDistance > 0) {
        ...
        const opacity = isWithinDistance ? 1.0 : 0.4;
        return { ...cluster, opacity, distance };
      }

      // If no user location or distance filter, show all at full opacity
      if (cluster.player && (!userLocation || !maxDistance || maxDistance <= 0)) {
        count++;
      }
      return { ...cluster, opacity: 1.0 };
    ...
    return { processedClusters: processed, withinDistanceCount: count };
  }, [clusters, userLocation, maxDistance]);

  // Update parent count in useEffect (not during render) to avoid React warning
  React.useEffect(() => {
    if (onCountUpdate) {
      onCountUpdate(withinDistanceCount);
    }
  }, [withinDistanceCount, onCountUpdate]);

  return (
    <>
      {/* 1. Server-side Pre-aggregated Clusters (Static) - with hover tooltip */}
      {processedClusters.filter(c => c.isCluster).map((cluster, idx) => {
      ...
      <MarkerClusterGroup
        ...
        maxClusterRadius={60}
        eventHandlers={{
          clusterclick: () => onPlayerSelect(null),
        }}
      >
        {processedClusters.filter(c => !c.isCluster).map((cluster, idx) => (
          <Marker
            ...
            icon={createCustomIcon(
              cluster.player?.sport || 'Cricket',
              cluster.player?.role === 'coach' ? 'coach' : 'player',
              cluster.opacity || 1.0
            )}
            eventHandlers={{
...
// Component to handle map clicks (not on markers)
// Markers will stop event propagation, so clicks on empty map area will trigger this
const MapClickHandler: React.FC<{ onMapClick?: () => void }> = ({ onMapClick }) => {
  useMapEvents({
    click: () => {
      // This fires when clicking on the map background (markers stop propagation)
      onMapClick?.();
    },
  });
  return null;
...
export const ScalableMap: React.FC<ScalableMapProps> = ({
  ...
  sport,
  viewMode,
  maxDistance = 5, // Default 5km if not provided
  userType = 'all'
}) => {
  ...
  const handleDataLoad = useCallback((newClusters: ClusterItem[], total: number) => {
    ...
    // Don't update onTotalChange here - will be updated by count callback
  }, []);

  const handleCountUpdate = useCallback((count: number) => {
    setPlayersWithinDistance(count);
    onTotalChange?.(count);
  }, [onTotalChange]);
  ...
  return (
    <div className="w-full h-full z-0 relative">
      ...
      >
        ...
        {/* Handle map clicks to close player card */}
        ...
        {hasGpsLocation && userLocation && (
          <>
            ...
            {/* Search Radius Circle - Visual feedback for distance filter */}
            <Circle
              ...
              pathOptions={{
                ...
                fillColor: '#E17827',
                fillOpacity: 0.1, // Very light fill (10% opacity)
      ...
      <style>{`
        ...
        /* Hide zoom control on mobile devices - show only on web view */
