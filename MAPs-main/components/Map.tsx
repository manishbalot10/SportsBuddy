  ...
  const processedClusters = React.useMemo(() => {
    ...
    const processed = clusters.map(cluster => {
      if (cluster.isCluster) {
        // For clusters, we can't determine exact distance, so show at full opacity
        // In a real implementation, you might want to calculate cluster center distance
      ...
      if (cluster.player && userLocation && maxDistance && maxDistance > 0) {
        ...
        // Reduce opacity to 0.4 for players outside the circle
      ...
      // Count all players when no distance filter is applied
      if (cluster.player && (!userLocation || !maxDistance || maxDistance <= 0)) {
        withinDistanceCount++;
      }
      return { ...cluster, opacity: 1.0 };
    ...
    return processed;
  }, [clusters, userLocation, maxDistance, onCountUpdate]);
